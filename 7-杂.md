...
- `bitset(const string& str)`: 设为 ![](data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7 "01") 串 `str`。
...
- 1. `flip()`: 翻转每一位。（![](data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7 "0\leftrightarrow1")，相当于异或一个全是 ![](data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7 "1") 的 `bitset`）
  2. `flip(pos)`: 翻转某一位。
...
咕宝主要问题在于不能陷入长时间无效思考状态，所以咕宝优先读题。

我和胜利爷有时候会在一些思路堵死，要及时跳出来误区。

榜上有简单题做不出来的时候，切记转换一下思路或立即拉另一个人过来重新想（先不要交流思路）

对于签到，和咕宝沟通下题意直接让他写。

对于铜银题，板子比较多的我来写，比较有细节的胜利爷来写，剩下的咕宝来写

对于金牌题，最后大伙一块all in，交给咕宝来写
