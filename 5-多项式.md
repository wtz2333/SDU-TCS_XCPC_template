...
$\prod_{i = 1}^nf_i(x)$，分治两两合并即可，要求每个多项式次数较低
...
auto solve = [&](auto &&self,int l,int r) -> void {
    ...
    int mid = l + r >> 1;
    self(self,l,mid);
    vector<int> tmp1(mid - l + 2),tmp2(r - l + 2);
    for(int i = l;i <= mid;i ++) tmp1[i - l + 1] = f[i];
    for(int i = 1;i <= r - l + 1;i ++) tmp2[i] = fac[i];
    auto c = (tmp1 * tmp2);
    for(int i = mid + 1;i <= r;i ++) f[i] = sub(f[i],c[i - l + 1]);
    self(self,mid + 1,r);
    return;
};
...
对于形如 $\prod(1 + x^{a_i})$ 这种形式的，对其取 ln 将乘法转为加法，然后再exp回去。
...
```cpp
void FWT_or(vector<Z> a,int N,int opt){ // N = 2^n
...
void FWT_and(vector<Z> a,int N,int opt){
    for(int len = 2,M = 1;len <= N;M = len,len <<= 1){
        for(int L = 0,R = len - 1; R <= N;L += len,R += len){
            for(int k = L;k < L + M;k ++){
                if(opt == 1)a[k] += a[k + M];
...
void FWT_xor(vector<Z> a,int N,int opt){ // a \in [0,N - 1]
    for(int len = 2,M = 1;len <= N;M = len,len <<= 1){
        for(int L = 0,R = len - 1; R <= N;L += len,R += len){
            for(int k = L;k < L + M;k ++){
                Z x = a[k],y = a[k + M];
                a[k] = x + y;a[k + M] = x - y;
                if(opt == -1)a[k] = a[k] * inv2,a[k + M] = a[k + M] * inv2;
            }
        }
    }// ifwt等于fwt后除N
}
...
斐波那契数列生成函数 $F(x) = \frac{x}{1-x-x^2} = \sum_{n\ge 0}(1-2^{n+1}+(n+1)\cdot 2^{n+1})x^{n}$

**五边形数**
$$
\prod_{i\ge1}(1-x^{i}) = \sum_{k=-\infty}^{\infty}(-1)^{k}x^{\frac{k(3k-1)}{2}}
$$
